        ...
        switch(command) {
         ...
         break
          case 'menu': case 'help':{
            ...
            alpha.sendMessage(m.chat, {image: pp_bot, caption: help.menu(pushname, salam, mundur, upload, download, totalGb, usedGb, freeGb, namaowner, namabot, jam, tanggal, runtime(process.uptime()), prefix)}, {quoted:m})
          }
          break
//anime
case "akame":
...
let res = await fetch(global.api('alfa', '/api/downloader/youtube-video', {url: args[0]}, 'apikey'))
            if (!res.ok) throw await res.text()
            var result = await res.json()             
            var { id, thumbnail, title, quality, filesize, size, download } = result.result   
            let but =  [{
   buttonId: '.menu',
   buttonText: {
      displayText: '⚡ Menu'
...
let key = "「 *YOUTUBE VIDEO* 」\n\n"
            ...
            await alpha.sendButImage(m.chat, thumbnail, but, key, footer_text, [], {
               ...
               alpha.sendMessage(from, {video:{url: download}, mimetype:"video/mp4", caption: mess.done}, {quoted:m})
         ...
         case 'ytmp3': {
            ...
            if (!args[0].includes('youtu.be') && !args[0].includes('youtube.com')) return reply(`Kirim perintah:\n${prefix+command} link youtube\n\nContoh penggunaan:\n${prefix+command} https://youtu.be/kwop2Eg5QY4`)
            reply(mess.wait)
let res = await fetch(global.api('alfa', '/api/downloader/youtube-audio', {url: args[0]}, 'apikey'))
            if (!res.ok) throw await res.text()
            var result = await res.json()             
            var { id, thumbnail, title, quality, filesize, size, download } = result.result  
            let but =  [{
   ...
   type: 1
            ...
            await alpha.sendButImage(m.chat, thumbnail, but, key, footer_text, [], {
               ...
               alpha.sendMessage(from, {audio:{url: download}, mimetype:"audio/mpeg"}, {quoted:m})
...
case 'play': {
            ...
            reply(mess.wait)
            let res = await fetch(global.api('alfa', '/api/downloader/youtube-search', {query: text}, 'apikey'))
            if (!res.ok) throw await res.text()
            var result = await res.json()
            var { videoId, image, title, views, duration, author, ago, url, description } = result.result.all[0] 
            let but =  [{
   buttonId: `.ytmp3 ${url}`,
   buttonText: {
      displayText: '🎧 Audio'
...
//if (isUrl(text)) return reply(`Kirim perintah:\n${prefix+command} query\n\nContoh penggunaan:\n${prefix+command} sakura`)
reply(mess.wait) 
let res = await fetch(global.api('alfa', '/api/downloader/pinterest2', {query: text}, 'apikey'))
if (!res.ok) throw await res.text()
...
reply(mess.wait) 
let res = await fetch(global.api('alfa', '/api/downloader/mediafire', {url: args[0]}, 'apikey'))
if (!res.ok) throw await res.text()
var result = await res.json()
var { nama, size, link } = result.result
if (size.replace('MB', '') >= 100 || size.replace('GB', '') >= 1){ //size edit sendiri jika mau download yang lebih media yang lebih besar
 ...
 alpha.sendMessage(from, {text: key}, {quoted:m})
} else {
 var key = `*「 Mediafire Download 」*\n\n` 
 key += `Nama: ${nama}\n`
 key += `Tipe: ${result.mime}\n`
...
await alpha.sendMessage(from, {text: key}, {quoted:m})
...
var result = await res.json()
var result = result.result 
alpha.sendMessage(from, {video:{url: result.audio}, mimetype:"video/mp4", caption: mess.done}, {quoted:m})
}
break
case "tiktokaudio":{
if (!args[0]) return reply(`Kirim perintah:\n${prefix+command} link tiktok video\n\nContoh penggunaan:\n${prefix+command} https://www.tiktok.com/@zeeone.official/video/7210229439744003355?_r=1&u_code=e44201c8bfkd30&region=ID&mid=7202111782981913370&preview_pb=0&language=id&_d=e0cah74j08m7c7&share_item_id=7210229439744003355&source=h5_t&timestamp=1679216331&user_id=7148061777321133083&sec_user_id=MS4wLjABAAAA50SieLfP2YD-R-gqSE3svcPxaPqr_53pA6RKyJUkQo_AreOGrLDiVRnajBVglVIk&utm_source=copy&utm_campaign=client_share&utm_medium=android&share_iid=7160625938232592154&share_link_id=6b2fea89-b038-4919-8d58-38b4efba5b9c&share_app_id=1180&ugbiz_name=Main&ug_btm=b8727%2Cb2878`)
...
var result = await res.json()
var result = result.result 
alpha.sendMessage(from, {audio:{url: result.audio}, mimetype:"audio/mpeg", caption: mess.done}, {quoted:m})
}
break
case "tiktokvn":{
if (!args[0]) return reply(`Kirim perintah:\n${prefix+command} link tiktok video\n\nContoh penggunaan:\n${prefix+command} https://www.tiktok.com/@zeeone.official/video/7210229439744003355?_r=1&u_code=e44201c8bfkd30&region=ID&mid=7202111782981913370&preview_pb=0&language=id&_d=e0cah74j08m7c7&share_item_id=7210229439744003355&source=h5_t&timestamp=1679216331&user_id=7148061777321133083&sec_user_id=MS4wLjABAAAA50SieLfP2YD-R-gqSE3svcPxaPqr_53pA6RKyJUkQo_AreOGrLDiVRnajBVglVIk&utm_source=copy&utm_campaign=client_share&utm_medium=android&share_iid=7160625938232592154&share_link_id=6b2fea89-b038-4919-8d58-38b4efba5b9c&share_app_id=1180&ugbiz_name=Main&ug_btm=b8727%2Cb2878`)
...
var result = await res.json()
var result = result.result 
alpha.sendMessage(from, {audio:{url: result.audio}, mimetype:"audio/mpeg", ptt:true, caption: mess.done}, {quoted:m})
}
break
//semoji
case "apple":
...
case "google": {
            reply(mess.wait)
            let res = await fetch(global.api('alfa', '/api/emoji/' + command, {emoji: args[0]}, 'apikey'))
            if (!res.ok) throw await res.text()
            let img = await res.buffer()
