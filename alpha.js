...
require('./case')
    ...
    alpha.ev.on('group-participants.update', async (anu) => {
        ...
        let hmm = await getBuffer(ppuser)
            ...
            let res = await fetch(global.api('alfa', '/api/canvas/welcomev2', {avatar:ppu, username: `${encodeURI(num.split("@")[0])}`, background: background, guildname: metadata.subject,membercount: `${encodeURI(participants.length)}`}, 'apikey'))
...
let goobye, ppu, ppgc;
        let hmm = await getBuffer(ppuser)
            ...
            let res = await fetch(global.api('alfa', '/api/canvas/goodbyev2', {avatar:ppu, username: `${encodeURI(num.split("@")[0])}`, background: background, guildname: metadata.subject,membercount: `${encodeURI(participants.length)}`}, 'apikey'))
            if (!res.ok) throw await res.text()
            try {
